...
import type { AgentRecommendation, CommitmentConfig, CommitmentReadModel, EvidenceSnapshot, EvidenceSource, PreparedAction } from "./types";
...
const DEFAULTS: Record<EvidenceSource, { observedValue: string; unit: string; deterministic: boolean; notes: string }> = {
  workouts: { observedValue: "completed workout", unit: "session", deterministic: true, notes: "Mocked from a workout or fitness API." },
  sleep: { observedValue: "8.1", unit: "hours", deterministic: true, notes: "Mocked from a sleep or readiness API." },
  "calendar-context": { observedValue: "blocked health event", unit: "event", deterministic: true, notes: "Mocked from calendar and schedule context." },
  "conversation-hints": { observedValue: "user reported strong intent", unit: "hint", deterministic: false, notes: "Mocked from conversational inference." },
};
...
const getPreparedAction = (
  ...
  if (action === "record_check_in") {
    return { action, contractAddress: input.contractAddress, functionName: "recordCheckIn", args: [input.commitmentId ?? 0], value: "0", explanation };
  }

  if (action === "settle_success") {
    return { action, contractAddress: input.contractAddress, functionName: "settleSuccess", args: [input.commitmentId ?? 0], value: "0", explanation };
  }

  if (action === "settle_failure") {
    return { action, contractAddress: input.contractAddress, functionName: "settleFailure", args: [input.commitmentId ?? 0], value: "0", explanation };
  }
